...
- **#4 Tavily evidence 沉淀**（2026-07-14 实施）：
  - `tavily_search.py`：`search_industry_news`/`search_with_ai_summary` 加 days 传参
  - `search_cache.py`：TTL 12h→720h（30 天）
  - `archive.py`：evidence_pool 分层淘汰（S/F 180 天/D 30 天），score_history 10→50
  - commit `5cf4e44`
...
## 已实现：每日板块共振自进化系统 (2026-07-14)

**`skills/daily_resonance/`** — 完整实现 SPEC_daily_resonance.md 全部 8 个 Phase：

| Phase | 内容 | 状态 |
|-------|------|------|
| 1 | `config.py` + `data.py` 数据加载 | ✅ |
| 2 | `agent1_classify.py` 关键词匹配+LLM兜底 | ✅ 含 Step 4 LLM兜底 |
| 3 | `agent2_resonance.py` 5维共振计算 | ✅ |
| 4 | `agent3_report.py` LLM/模板报告 | ✅ |
| 5 | `evolution.py` 自进化+收敛+市场结构检测 | ✅ |
| 6 | `__main__.py` CLI入口 | ✅ `--date/--json/--no-evolve/--no-llm` |
| 7 | `scripts/backtest_resonance.py` 回测脚本 | ✅ |
| 8 | cron配置+CLAUDE.md文档 | ✅ |

**关键设计决策**:
- Agent 1 LLM兜底：关键词不匹配的新闻(≤100条/天)用 `get_llm_client().synthesize()` 批量分类
- 自进化：贝叶斯学习率衰减，连续14天Δw<0.01+准确率≥55%收敛
- 日成本约¥0.10（Agent 1兜底¥0.02 + Agent 3报告¥0.08）
- 完全独立于 chain_agent（只读不写）
...
- #2 双 key 清理：✅ 已做
- #4 Tavily shared days + 沉淀：✅ 已做（2026-07-14，commit `5cf4e44`）
- #5 迁移工具：✅ 已做
