"""
us-deep-analyze skill CLI（美股版）

用法:
  # chain 模式
  python -m skills.us-deep-analyze --chain semiconductors
  python -m skills.us-deep-analyze --chain ai_cloud --days 14 --top-n 8 --out report.md

  # stock 模式
  python -m skills.us-deep-analyze --stock AAPL
  python -m skills.us-deep-analyze --stock NVIDIA --out verdict.md
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="us-deep-analyze skill: 美股产业链深度拆解 + 投资价值判断",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--chain", type=str, help="美股产业链名（英文 key，如 semiconductors / ai_cloud / consumer_electronics）")
    group.add_argument("--stock", type=str, help="美股 ticker 或公司名（如 AAPL / NVIDIA）")
