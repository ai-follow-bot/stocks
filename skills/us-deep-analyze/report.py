"""us-deep-analyze 报告渲染（Markdown，美股版）

基于 skills/deep-analyze/report.py 改写：
- 表头 "国替(30)" → "全球竞争力(30)"
- "国替率" → "美股市占率"
- akshare → Finnhub
- 其他渲染逻辑复用（市场无关）
...
def render_chain_report(result: dict) -> str:
    ...
    lines = [
        f"# {chain.get('chain_name', result.get('chain_name', ''))} 美股产业链深度拆解",
        f"\n*生成时间: {result.get('run_time')} | 数据窗口: {result.get('days')} 天 | 市场: 美股*",
    ]
    if data_quality == "degraded":
        lines.append("\n> ⚠️ **数据降级**：所有搜索源（Tavily+智谱+Finnhub）均失败，"
                     "本次评分基于 LLM 知识估算，置信度低。\n")
    lines.append("\n## 1. 产业链结构\n")
    lines.append("| 环节 | 上下游 | 全球龙头 | 美股龙头 | 集中度 | 美股市占率 | 技术门槛 |")
    lines.append("|------|--------|---------|---------|--------|-----------|---------|")
    for s in chain.get("segments", []):
        lines.append(
            f"| {s.get('name','')} | {s.get('role','')} | "
            f"{', '.join(s.get('global_leaders', [])[:2]) or '-'} | "
            f"{', '.join(s.get('us_leaders', [])[:2]) or '-'} | "
            f"{s.get('concentration','-')} | {s.get('us_share','-')} | "
    ...
    if bn_segments:
        lines.append("\n## 2. 卡脖子分析\n")
        lines.append("| 环节 | 集中度 | 美股提升空间 | 技术门槛 | 价格信号 | 总分 | 卡脖子? | 提取数字 |")
        lines.append("|------|--------|-------------|---------|---------|------|---------|---------|")
        for s in bn_segments:
            ...
            if nums.get("us_share"):
                nums_str += f"美股市占率={nums['us_share']}"
            nums_str = nums_str.strip() or "-"
            lines.append(
                f"| {s.get('name','')} | {s.get('supply_concentration','-')} | "
                f"{s.get('us_substitution_room','-')} | {s.get('tech_barrier','-')} | "
    ...
    if candidates:
        lines.append("\n## 3. 三维评分（候选标的）\n")
        lines.append("| 标的 | 代码 | 环节 | 供需(30) | 全球竞争力(30) | 业绩(40) | 总分 | 权重 | PE | 市值(亿美元) |")
        lines.append("|------|------|------|---------|---------------|---------|------|------|-----|-------------|")
        # 按 total_score 排序
        def _total(c):
        ...
        for c in candidates_sorted:
            ...
            lines.append(
                ...
                f"{sc.get('global_competitiveness','-')} | {sc.get('earnings_realization','-')} | "
        ...
        for c in candidates_sorted:
            ...
            if rat.get("global_competitiveness_reason"):
                lines.append(f"- 全球竞争力: {sc.get('global_competitiveness','-')} — {rat['global_competitiveness_reason']}")
    ...
    gaps = []
    if data_quality == "degraded":
        gaps.append("- ⚠️ 所有搜索源（Tavily+智谱+Finnhub）失败，评分基于 LLM 知识估算")
    if not any(d.get("tavily_count", 0) > 0 for d in search_stats.values()):
        if data_quality != "degraded":
            gaps.append("- 网络搜索不可用（Tavily+智谱均失败），所有环节缺深度搜索数据")
    low_news = [seg for seg, d in search_stats.items() if d.get("finnhub_news_count", 0) == 0]
    if low_news:
        gaps.append(f"- 以下环节 Finnhub 新闻命中为 0: {', '.join(low_news)}")
    if not gaps:
        gaps.append("- 暂无明显缺口")
    lines.extend(gaps)
...
def render_stock_verdict(result: dict) -> str:
    ...
    lines = [
        f"# {result.get('stock_name')}（{result.get('stock_code')}）美股投资判断",
        f"\n*生成时间: {result.get('run_time')} | 市场: 美股*",
    ]
    if data_quality == "degraded":
